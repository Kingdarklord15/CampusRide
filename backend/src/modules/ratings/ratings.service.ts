...
export class RatingsService {
  ...
  async submit(userId: string, data: { rideId: string; score: number; comment?: string }) {
    const ride = await this.prisma.ride.findUnique({ where: { id: data.rideId }, include: { passengerProfile: true } });
    if (!ride || !ride.driverProfileId) throw notFound("Completed ride with driver not found");
    if (ride.status !== "COMPLETED") throw badRequest("Only completed rides can be rated");
    if (ride.passengerProfile.userId !== userId) throw badRequest("Only the passenger can rate this ride");
  ...
  driverRatings(driverProfileId: string) {
