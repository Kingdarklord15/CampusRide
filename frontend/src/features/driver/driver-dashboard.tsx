...
export function DriverDashboard() {
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-white p-4"><div><p className="font-semibold">Driver availability</p><p className="text-sm text-zinc-500">Current status: {status}</p></div><Button onClick={async () => { const next = status === "ONLINE" ? "OFFLINE" : "ONLINE"; await driverService.setStatus(next); setStatus(next); }}>{status === "ONLINE" ? "Go offline" : "Go online"}</Button></div>
