...
export function RegisterForm() {
  ...
  return (
    <form className="grid gap-4" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
      ...
      {mutation.isError ? <p className="text-sm text-rose-600">Registration failed.</p> : null}
      <Button disabled={mutation.isPending}>{mutation.isPending ? "Creating..." : "Create account"}</Button>
      <Link href="/login" className="text-center text-sm text-indigo-600">Already have an account?</Link>
    </form>
  );
}
